...
class FilesytemUI extends FilesystemUI_Abstract {
  ...
  async invokeFileBrowser (
    ...
    let selectionResult
    try {
      selectionResult = await FileSelect.select({
        multiple: false,
        extensions: ["images"]  // ["*"] // alow any file type
      })
    } catch (e) {
      if (e && e.message === 'canceled') return null
      else throw e
    }
    let selectedFile = selectionResult.files[0]

    if (Capacitor.platform == 'android') {
      console.log('FileSelection: platform is Android')
      console.log(`Selected file: ${selectedFile.path} ${selectedFile.name}`)
      let scrubbedFilePath = selectedFile.path.split('capacitor_file_/')[1]
      console.log(scrubbedFilePath)
      let data = await Filesystem.readFile({
        path: 'file:///' + scrubbedFilePath
      })
      console.log('Here we go with the data from the file')
      console.log(data)
      console.log(data.data)
      console.log(`Extension: ${selectedFile.extension}`)
      let imgUri = "data:image/" + selectedFile.extension + ";base64," + data.data
