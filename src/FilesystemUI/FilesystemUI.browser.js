...
class FilesytemUI extends FilesystemUI_Abstract {
  ...
  async PresentDialogToOpenOneImageFile (
    ...
    const self = this
    const params = { title, fn }
    console.log('FSUI: Present Dialog')
    const fileSelection = await self.invokeFileBrowser().then((returnData) => {
    ...
    const decodedFile = btoa(fileSelection)
  ...
  async invokeFileBrowser (
    ...
    const self = this
    console.log(self.context.deviceInfo);
    console.log(self.context.deviceInfo.platform == 'web');
    if (self.context.deviceInfo.platform == 'web') {
    ...
    FilePicker.showFilePicker({
      ...
      async (fileResult) => {
        ...
        // const pathArr = paths[0].split('capacitor_file_/')
        // console.log(pathArr)
