...
function New_copyButton_aLayer (context, value__orValuesByContentType, enabled_orTrue, pasteboard) { // defaults to 'text' content type
  ...
  function _setValueToCopy (to_value__orValuesByContentType) {
    ...
    if (typeof to_value__orValuesByContentType === 'string') {
      ...
      for (let key in to_value__orValuesByContentType) { // set the very last value .. which could end up being text, or maybe html.... TODO: if this ever actually gets hit in any cases, this can be fixed up
...
function New_copyable_longStringValueField_component_fieldContainerLayer (
  ...
  let labelLayer = New_fieldTitle_labelLayer(fieldLabelTitle, context)
  ...
  let valueLayer = New_fieldValue_labelLayer('' + valueToDisplay, context)
